...
const FIRST_REFERENCE_HASH: ChunkHash = ChunkHash {
    bytes: [
        0x61, 0x16, 0xc6, 0x71, 0xdc, 0xa1, 0x0a, 0xb1,
        0xaa, 0x88, 0xa1, 0x41, 0x0c, 0x28, 0x92, 0xde,
        0x72, 0x85, 0xb7, 0x39, 0x17, 0x98, 0x47, 0xbc,
        0xe4, 0x5c, 0xdc, 0xd8, 0x45, 0x99, 0x09, 0x4a,
    ],
};
...
const SECOND_REFERENCE_HASH: ChunkHash = ChunkHash {
    bytes: [
        0x6b, 0x3b, 0x80, 0xfa, 0x91, 0x55, 0x25, 0x26,
        0x6f, 0x8a, 0x5b, 0x31, 0x8c, 0xd8, 0x25, 0xaf,
        0xc5, 0x8e, 0xf1, 0xfd, 0xbc, 0xd5, 0x53, 0x51,
        0x07, 0xc9, 0xcc, 0xe0, 0x64, 0x62, 0x52, 0xef,
    ],
};
...
fn parses_the_specification_example() {
    assert_eq!(parse_xet_hash(SPEC_EXAMPLE_TEXT).unwrap(), SPEC_EXAMPLE_HASH);
}

#[test]
fn parses_published_reference_hashes() {
    assert_eq!(parse_xet_hash(FIRST_REFERENCE_TEXT).unwrap(), FIRST_REFERENCE_HASH);
    assert_eq!(parse_xet_hash(SECOND_REFERENCE_TEXT).unwrap(), SECOND_REFERENCE_HASH);
}

#[test]
fn raw_hash_round_trips_through_xet_text() {
    for hash in [SPEC_EXAMPLE_HASH, FIRST_REFERENCE_HASH, SECOND_REFERENCE_HASH] {
...
fn xet_text_round_trips_through_raw_hash() {
    for text in [SPEC_EXAMPLE_TEXT, FIRST_REFERENCE_TEXT, SECOND_REFERENCE_TEXT] {
...
fn parser_rejects_noncanonical_text() {
    ...
    assert!(parse_xet_hash(
        "g10aa1dc71c61661de92280c41a188aabc47981739b785724a099945d8dc5ce4"
    )
    .is_err());
    assert!(parse_xet_hash(
        "B10AA1DC71C61661DE92280C41A188AABC47981739B785724A099945D8DC5CE4"
